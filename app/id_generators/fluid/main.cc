...
void set_derived_params() {
  ...
  if(gdimension == 3){
    totalmass = rho_1*abs(box_max[0]-box_min[0])*abs(box_min[1]-box_max[1])*
      abs(box_min[2]-box_max[2]);
  }
  if(gdimension == 2){
    totalmass = rho_1*abs(box_max[0]-box_min[0])*abs(box_min[1]-box_max[1]);
  }
  ...
  np = particle_lattice::count(lattice_type,gdimension,box_min,box_max,
                                      sph_separation, 0);
