...
)
{
  ...
  if(H5P_hasStep(file_id,step)){
    group_id = H5Gopen(file_id, cstep, H5P_DEFAULT );
  }else{
    group_id = H5Gcreate(file_id, cstep, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  }
  std::cout<<"done"<<std::endl;
  H5Pclose(plist_id);
}
  ...
  size_t dim)
{
  ...
  * H5Pset_dxpl_mpio(plist_id, H5FD_MPIO_INDEPENDENT);*/
  if(typeid(T) == typeid(int)){
    dset_id = H5Dcreate(group_id, dsname, H5T_NATIVE_INT, filespace,
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    status = H5Dwrite(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
      plist_id, data);
  }else if(typeid(T) == typeid(double)){
    dset_id = H5Dcreate(group_id, dsname, H5T_NATIVE_DOUBLE, filespace,
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    status = H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL,
      plist_id, data);
  }else if (typeid(T) == typeid(int64_t)){
    dset_id = H5Dcreate(group_id, dsname, H5T_NATIVE_LLONG, filespace,
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    status = H5Dwrite(dset_id, H5T_NATIVE_LLONG, H5S_ALL, H5S_ALL,
      plist_id, data);
  }else if (typeid(T) == typeid(uint64_t)){
    dset_id = H5Dcreate(group_id, dsname, H5T_NATIVE_ULLONG, filespace,
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    status = H5Dwrite(dset_id, H5T_NATIVE_ULLONG, H5S_ALL, H5S_ALL,
      plist_id, data);
  } else {
  ...
  T* data)
{
  ...
  hid_t status = 1;
  hid_t att_id = H5Aopen(group_id,dsname,H5P_DEFAULT);
  /*Create property list for collective dataset write.*/
  hid_t plist_id = H5Pcreate(H5P_DATASET_XFER);
  H5Pset_dxpl_mpio(plist_id, H5FD_MPIO_COLLECTIVE);
  /* To write dataset independently use
  ...
  size_t dim)
{
  ...
  //hid_t filespace = H5Screate_simple(1, &hdim, NULL);
  /* Open the dataset*/
  hid_t dset_id = H5Dopen (group_id, dsname, H5P_DEFAULT);
  /*Create property list for collective dataset write.*/
  hid_t plist_id = H5Pcreate(H5P_DATASET_XFER);
  H5Pset_dxpl_mpio(plist_id, H5FD_MPIO_COLLECTIVE);
  /* To write dataset independently use
  * H5Pset_dxpl_mpio(plist_id, H5FD_MPIO_INDEPENDENT);*/
  if(typeid(T) == typeid(int)){
    ...
    status = H5Dread(dset_id, H5T_NATIVE_ULLONG, H5S_ALL, H5S_ALL,
      plist_id, data);
  } else {
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Finalize();
...
H5P_getNumParticles(hid_t file_id)
{
  ...
  hid_t dset_id = H5Dopen (group_id, "x", H5P_DEFAULT);
  ...
  int startIteration)
{
  ...
  std::cout<<"NPARTPROC ="<<nparticlesproc<<std::endl;
  ...
  // Positions
  errX = H5P_readDataset(dataFile,"x",dataX,nparticlesproc);
  if(gdimension > 1)
    errY = H5P_readDataset(dataFile,"y",dataY,nparticlesproc);
  if(gdimension > 2)
    errZ = H5P_readDataset(dataFile,"z",dataZ,nparticlesproc);
  ...
  // Velocity
  errX = H5P_readDataset(dataFile,"vx",dataX,nparticlesproc);
  if(gdimension > 1)
    errY = H5P_readDataset(dataFile,"vy",dataY,nparticlesproc);
  if(gdimension > 2)
    errZ = H5P_readDataset(dataFile,"vz",dataZ,nparticlesproc);
  ...
  // Acceleration
  errX = H5P_readDataset(dataFile,"ax",dataX,nparticlesproc);
  if(gdimension > 1)
    errY = H5P_readDataset(dataFile,"ay",dataY,nparticlesproc);
  if(gdimension > 2)
    errZ = H5P_readDataset(dataFile,"az",dataZ,nparticlesproc);
  ...
  errX = H5P_readDataset(dataFile,"m",dataX,nparticlesproc);
  errY = H5P_readDataset(dataFile,"rho",dataY,nparticlesproc);
  errZ = H5P_readDataset(dataFile,"h",dataZ,nparticlesproc);
  ...
  // Pressure
  errX = H5P_readDataset(dataFile,"P",dataX,nparticlesproc);
  if(errX != 0)
  ...
  std::fill(dataX,dataX+nparticlesproc,0.);
  errX = H5P_readDataset(dataFile,"u",dataX,nparticlesproc);
  if(errX != 0)
  ...
  bool b_index = 0 ==
  H5P_readDataset(dataFile,"id",dataInt,nparticlesproc);
  if(b_index){
    for(int64_t i=0; i<nparticlesproc; ++i){
  ...
  // delta T
  errX = H5P_readDataset(dataFile,"dt",dataX,nparticlesproc);
  if(errX != 0)
  ...
  // delta T
  errX = H5P_readDataset(dataFile,"type",dataInt32,nparticlesproc);
  if(errX != 0)
    ...
    double totaltime)
{
  ...
  H5P_writeDataset(dataFile,"x",b1,nparticlesproc);
  H5P_writeDataset(dataFile,"y",b2,nparticlesproc);
  H5P_writeDataset(dataFile,"z",b3,nparticlesproc);
  ...
  H5P_writeDataset(dataFile,"vx",b1,nparticlesproc);
  H5P_writeDataset(dataFile,"vy",b2,nparticlesproc);
  H5P_writeDataset(dataFile,"vz",b3,nparticlesproc);
  ...
  H5P_writeDataset(dataFile,"ax",b1,nparticlesproc);
  H5P_writeDataset(dataFile,"ay",b2,nparticlesproc);
  H5P_writeDataset(dataFile,"az",b3,nparticlesproc);
  ...
  H5P_writeDataset(dataFile,"h",b1,nparticlesproc);
  H5P_writeDataset(dataFile,"rho",b2,nparticlesproc);
#ifdef INTERNAL_ENERGY
  H5P_writeDataset(dataFile,"u",b3,nparticlesproc);
#endif
 ...
 // Pressure, Mass, Id, timestep
  ...
  H5P_writeDataset(dataFile,"P",b1,nparticlesproc);
  H5P_writeDataset(dataFile,"m",b2,nparticlesproc);
  H5P_writeDataset(dataFile,"dt",b3,nparticlesproc);
  H5P_writeDataset(dataFile,"id",bi,nparticlesproc);
  H5P_writeDataset(dataFile,"type",bint,nparticlesproc);

  // Output the rank for analysis
  std::fill(bi,bi+nparticlesproc,rank);
  H5P_writeDataset(dataFile,"rank",bi,nparticlesproc);
