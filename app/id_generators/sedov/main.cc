...
main(int argc, char * argv[]) {
  ...
  if(not modify_initial_data) {
    ...
    auto _np = particle_lattice::generate(lattice_type, domain_type, bbox_min,
      bbox_max, sph_separation, 0, x, y, z);
  ...
  log_one(info) << "Total number of seeded blast particles: " << particles_blast
                << std::endl;
  log_one(info) << "Mass of seeded blast particles: " << mass_blast
                << std::endl;
