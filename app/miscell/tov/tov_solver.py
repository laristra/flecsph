...
dr = 5.0 # Radial step
filename = 'tov_pp_{}_particles.h5'.format(num_particles)
...
with h5py.File(filename,'w') as f:
	xdset = f.create_dataset('particles/x',data = xpart)
	ydset = f.create_dataset('particles/y',data = ypart)
	zdset = f.create_dataset('particles/z',data = zpart)
	udset = f.create_dataset('particles/internal_energy',data = upart)
	vxdset = f.create_dataset('particles/v_x',data = vxpart)
	vydset = f.create_dataset('particles/v_y',data = vypart)
	vydset = f.create_dataset('particles/v_z',data = vzpart)
	mdset = f.create_dataset('particles/mass',data = mparticles)
	hdset = f.create_dataset('particles/smoothing_length',data = hparticles)
	mbdset = f.create_dataset('particles/mass_ball',data = particle_mball)
	r_a_dset = f.create_dataset('grid/r',data = r)
	rho_a_dset = f.create_dataset('grid/density',data = rho)
	m_a_dset = f.create_dataset('grid/mass_ball',data = mball)
	P_a_dset = f.create_dataset('grid/pressure',data = P)
	u_a_dset = f.create_dataset('grid/internal-energy.data',data = u)
