...
for i in `seq 1 $max`; do
  pwd
  dir_mpi="mpi${num}_${OMP_THREADS}_weak"
...
#  echo "#SBATCH --nodelist=cn[314-329]" >> task.sh
  ...
  echo "time mpirun -n $num --bind-to socket --map-by socket --report-bindings ../../../bin/drivers/hydro_3d ./sodtube_n$NPARTS.par" >> task.sh
