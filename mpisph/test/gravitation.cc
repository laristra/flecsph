...
TEST(tree_colorer, mpi_qsort){
  ...
  std::cout<<"test pSink="<<sinkPosition<<" pSource="<<sourcePosition<<std::endl;
  ...
  for(int i = 0; i<27; ++i){
    if(i<3){
      std::cout<<"fc i="<<i<<": code="<<fc[i]<<" res="<<fc_res[i]<<std::endl;
      ASSERT_TRUE(fabs(fc[i]-fc_res[i]) < 1.0e-10);
    }
    if(i<9){
      std::cout<<"dfcdr i="<<i<<": code="<<jacobi[i]<<" res="<<jacobi_res[i]<<std::endl;
      ASSERT_TRUE(fabs(jacobi[i]-jacobi_res[i]) < 1.0e-10);
    }
    std::cout<<"dfcdrdr i="<<i<<": code="<<hessian[i]<<" res="<<hessian_res[i]<<std::endl;
  ...
  for(int i = 0; i<27; ++i){
    if(i<3){
      std::cout<<"fc i="<<i<<": code="<<fc[i]<<" res="<<fc_res_2[i]<<std::endl;
      ASSERT_TRUE(fabs(fc[i]-fc_res_2[i]) < 1.0e-4);
    }
    if(i<9){
      std::cout<<"dfcdr i="<<i<<": code="<<jacobi[i]<<" res="<<jacobi_res_2[i]<<std::endl;
      ASSERT_TRUE(fabs(jacobi[i]-jacobi_res_2[i]) < 1.0e-4);
    }
    std::cout<<"dfcdrdr i="<<i<<": code="<<hessian[i]<<" res="<<hessian_res_2[i]<<std::endl;
  ...
  tfmm.computeAcceleration(sinkPosition,sourcePosition,sourceMass,fc,
    ...
    for(int i = 0; i<27; ++i){
    if(i<3){
      std::cout<<"fc i="<<i<<": code="<<fc[i]<<" res="<<fc_res_3[i]<<std::endl;
      ASSERT_TRUE(fabs(fc[i]-fc_res_3[i]) < 1.0e-4);
    }
    if(i<9){
      std::cout<<"dfcdr i="<<i<<": code="<<jacobi[i]<<" res="<<jacobi_res_3[i]<<std::endl;
      ASSERT_TRUE(fabs(jacobi[i]-jacobi_res_3[i]) < 1.0e-4);
    }
    std::cout<<"dfcdrdr i="<<i<<": code="<<hessian[i]<<" res="<<hessian_res_3[i]<<std::endl;
