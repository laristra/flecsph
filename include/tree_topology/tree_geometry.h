 ...
 *-----------------------------------------------------------------------------*/
 ...
 struct tree_geometry<T, 1>
 {
   ...
   //! center with radius.
   static bool within(
     const point_t& origin, const point_t& center,
     element_t r1, element_t r2 = 0)
   {
     return distance(origin, center) - r1 <= tol ;
   }

   //! Return true if dist^2 < radius^2
   static bool within_square(
     const point_t& origin, const point_t& center,
   ...
   static bool intersects_box_box(
     const point_t& min_b1, const point_t& max_b1,
     const point_t& min_b2, const point_t& max_b2)
   {
     return
     !((max_b1[0] < min_b2[0]) || (max_b2[0] < min_b1[0]));
   }

   //! Intersection of two spheres based on center and radius
   static bool intersects_sphere_sphere(
     const point_t& c1, const element_t r1,
   ...
   static bool intersects_sphere_box(
     const point_t& min, const point_t& max,
...
struct tree_geometry<T, 2>
{
  ...
  {
    return
    !((max_b1[0] < min_b2[0] || max_b1[1] < min_b2[1])
...
struct tree_geometry<T, 3>
{
  ...
  //! Return true if dist^2 < radius^2
  inline static bool within_square(
    const point_t& origin, const point_t& center,
  ...
  //! Intersection between two boxes defined by there min and max bound
  inline static bool intersects_box_box(
    const point_t& min_b1, const point_t& max_b1,
    const point_t& min_b2, const point_t& max_b2)
  {
    return
    ...
    const point_t& c2, const element_t r2)
  {
    return (c2[0]-c1[0])*(c2[0]-c1[0])+(c2[1]-c1[1])*(c2[1]-c1[1])+
          (c2[2]-c1[2])*(c2[2]-c1[2]) <= (r1+r2)*(r1+r2);
  }
  ...
  static bool intersects_sphere_box(
    ...
    const point_t& c, const element_t& r)
  {
    point_t x = point_t(c[0]<max[0]?c[0]:max[0],c[1]<max[1]?c[1]:max[1],
                        c[2]<max[2]?c[2]:max[2]);
