...
const double quintic_spline_sigma[3] = {
  1. / 40., 63. / (478. * M_PI), 81. / (359. * M_PI)};
const double wendland_c2_sigma[3] = {1.25, 7. / M_PI, 21. / (2. * M_PI)};
const double wendland_c4_sigma[3] = {1.5, 9. / M_PI, 495. / (32. * M_PI)};
const double wendland_c6_sigma[3] = {
  55. / 32., 78. / (7. * M_PI), 1365. / (64. * M_PI)};
...
const double super_gaussian_sigma[3] = {
  1.69225265632490690298438831093977954454948887037797715437044546925320L, // ~
                                                                           // 3/sqrt(M_PI)
  2.86196342089351035329931251213987772907929746530058751630485064825889L, // ~
                                                                           // 9/M_PI
...
void
set_sinc_kernel_normalization(const double & si) {
  double sq = sqrt(si);
  double s2 = si * si;
  sinc_sigma[0] =
    2. * (si_b0[0] + si_b1[0] * sq + si_b2[0] * si + si_b3[0] / sq);
  sinc_sigma[1] =
    4. * (si_b0[1] + si_b1[1] * si + si_b2[1] / si + si_b3[1] / s2);
  sinc_sigma[2] =
    8. * (si_b0[2] + si_b1[2] * sq + si_b2[2] * si + si_b3[2] * sq * si);
}
...
template<param::sph_kernel_keyword K, int D>
point_t kernel_gradient(const point_t & pos, const double & h);
...
template<>
double
kernel<param::cubic_spline, gdimension>(const double & r, const double & h) {
  double rh = 2. * r / h;
  double result = 0.;
  ...
  if(rh < 2.0) {
    ...
    result *= cubic_spline_sigma[gdimension - 1] / pow(h, gdimension);
  }
  return result;
} // kernel
...
kernel_gradient<param::cubic_spline, gdimension>(const point_t & vecP,
  ...
  if(rh < 2.0) {
    double dWdr;
    double sigma =
      2. * cubic_spline_sigma[gdimension - 1] / pow(h, gdimension + 1);
    if(rh < 1.0)
...
template<>
double
kernel<param::gaussian, gdimension>(const double & r, const double & h) {
  double rh = 3. * r / h;
...
kernel_gradient<param::gaussian, gdimension>(const point_t & vecP,
  ...
  point_t result = 0.0;
  if(rh < 3.0) {
    double sigma = 3. * gaussian_sigma[gdimension - 1] / pow(h, gdimension + 1);
...
template<>
double
kernel<param::quintic_spline, gdimension>(const double & r, const double & h) {
  double rh = 3. * r / h;
...
kernel_gradient<param::quintic_spline, gdimension>(const point_t & vecP,
  ...
  if(rh < 3.) {
    double sigma =
      3. * quintic_spline_sigma[gdimension - 1] / pow(h, gdimension + 1);
...
template<>
double
kernel<param::wendland_c2, 1>(const double & r, const double & h) {
  double rh = r / h;
...
template<>
double
kernel<param::wendland_c2, 2>(const double & r, const double & h) {
  double rh = r / h;
  double hd = h * h;
  double rh2 = (1 - rh) * (1 - rh);
...
template<>
double
kernel<param::wendland_c2, 3>(const double & r, const double & h) {
  double rh = r / h;
  double hd = h * h * h;
  double rh2 = (1 - rh) * (1 - rh);
...
template<>
double
kernel<param::wendland_c4, 1>(const double & r, const double & h) {
  double rh = r / h;
  double rh2 = (1 - rh) * (1 - rh);
  double rh3 = rh2 * (1 - rh);
...
template<>
double
kernel<param::wendland_c4, 2>(const double & r, const double & h) {
  double rh = r / h;
  double hd = h * h;
  double rh1 = 1 - rh;
  double rh2 = rh1 * rh1;
...
template<>
double
kernel<param::wendland_c4, 3>(const double & r, const double & h) {
  double rh = r / h;
  double hd = h * h * h;
  double rh1 = 1 - rh;
  double rh2 = rh1 * rh1;
...
template<>
double
kernel<param::wendland_c6, 1>(const double & r, const double & h) {
  double rh = r / h;
  double rh2 = (1 - rh) * (1 - rh);
  double rh3 = rh2 * (1 - rh);
  double rh4 = rh2 * rh2;
...
template<>
double
kernel<param::wendland_c6, 2>(const double & r, const double & h) {
  double rh = r / h;
...
template<>
double
kernel<param::wendland_c6, 3>(const double & r, const double & h) {
  double rh = r / h;
  double hd = h * h * h;
  double rh2 = (1 - rh) * (1 - rh);
  double rh4 = rh2 * rh2;
...
template<>
double
kernel<param::super_gaussian, gdimension>(const double & r, const double & h) {
  double rh = 3. * r / h, rh2 = rh * rh;

  double sigma = super_gaussian_sigma[gdimension - 1] / pow(h, gdimension);
...
template<>
double
kernel<param::sinc_ker, gdimension>(const double & r, const double & h) {
...
select() {
  ...
  if(sph_kernel == cubic_spline or sph_kernel == wendland_c2 or
     sph_kernel == wendland_c4 or sph_kernel == wendland_c6) {
    kernel_width = 2.0;
  }
  ...
  else if(sph_kernel == quintic_spline or sph_kernel == gaussian or
          sph_kernel == super_gaussian) {
    kernel_width = 3.0;
  }
  else {
    log_fatal("Bad kernel parameter" << std::endl);
