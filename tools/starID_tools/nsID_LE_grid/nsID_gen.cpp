...
const double Kconstant =
  (radiusCM * radiusCM * 2 * M_PI * gravConst) / (ksi_1 * ksi_1) * K_ratio;
...
const double centralDensity =
  (mass * Aconstant * Aconstant * Aconstant) /
  (4. * M_PI *
    (sin(Aconstant * radiusCM) -
      Aconstant * radiusCM * cos(Aconstant * radiusCM)));
...
void
generate_position(std::vector<particle> & v,
  int64_t ntarget,
  double part_dist) {
...
compute_smoothing_length(int64_t nparticles) {
  Hconstant =
    1. / 2. * radiusCM * pow(nneighbs * 1. / nparticles, 1. / 3.) * H_ratio;
}
...
compute_particles_mass(std::vector<particle> & v) {
  ...
  for(int i = 0; i < nparticles; ++i) {
    if(v[i].x_ == 0. && v[i].y_ == 0. && v[i].z_ == 0.) {
      ...
      v[i].density_ = density(radiusSphere(v[i].x_, v[i].y_, v[i].z_));
    }
...
void
write_dataset(hid_t file, const char * name, std::vector<double> data) {
  hsize_t size = data.size();
  hid_t space_id = H5Screate_simple(1, &size, NULL);
  hid_t dataset = H5Dcreate(file, name, H5T_IEEE_F64LE, space_id, H5P_DEFAULT,
    H5P_DEFAULT, H5P_DEFAULT);

  herr_t status =
    H5Dwrite(dataset, H5T_IEEE_F64LE, H5S_ALL, H5S_ALL, H5P_DEFAULT, &data[0]);
  status = H5Sclose(space_id);
  status = H5Dclose(dataset);
}

void
write_dataset_int(hid_t file, const char * name, std::vector<int> data) {
  hsize_t size = data.size();
  hid_t space_id = H5Screate_simple(1, &size, NULL);
  hid_t dataset = H5Dcreate(file, name, H5T_NATIVE_INT, space_id, H5P_DEFAULT,
    H5P_DEFAULT, H5P_DEFAULT);

  herr_t status =
    H5Dwrite(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &data[0]);
  status = H5Sclose(space_id);
  status = H5Dclose(dataset);
}
...
void
write_attribute(hid_t file, const char * name, double value) {
  hsize_t size = 1;
  hid_t space_id = H5Screate_simple(1, &size, NULL);
  hid_t attribute =
    H5Acreate(file, name, H5T_IEEE_F64LE, space_id, H5P_DEFAULT, H5P_DEFAULT);
...
void
compute_angular_momentum(std::vector<particle> & v) {
  QZZ = 0.;
  for(auto & p : v) {
...
main(int argc, char * argv[]) {
  ...
  particles.erase(remove_if(particles.begin(), particles.end(),
                    [](const auto & e) -> bool { return e.mass_ < 1.0e-10; }),
    particles.end());

  nparticles = particles.size();
  std::cout << std::endl
            << "Particles after cleaning = " << nparticles << std::endl;
  ...
  auto dataFile = H5Fcreate(name, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);

  auto step_id =
    H5Gcreate(dataFile, "/Step#0", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
>>>>>>> master
  ...
  std::cout << "X    [" << *std::min_element(data1.begin(), data1.end()) << ","
            << *std::max_element(data1.begin(), data1.end()) << "]"
            << std::endl;

  std::cout << "Y    [" << *std::min_element(data2.begin(), data2.end()) << ","
            << *std::max_element(data2.begin(), data2.end()) << "]"
            << std::endl;

  std::cout << "Z    [" << *std::min_element(data3.begin(), data3.end()) << ","
            << *std::max_element(data3.begin(), data3.end()) << "]"
            << std::endl;
