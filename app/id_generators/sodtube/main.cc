  ...
  if constexpr(gdimension == 3) {
    if(not(periodic_boundary_x and periodic_boundary_y and periodic_boundary_z))
      log_one(warn)
        ...
        << std::endl
        << "  periodic_boundary_x = yes" << std::endl
        << "  periodic_boundary_y = yes" << std::endl
        << "  periodic_boundary_z = yes" << std::endl
        << "for best results." << std::endl;
  }
...
main(int argc, char * argv[]) {
  ...
  // screen output
  log_one(info) << "Number of particles: " << tparticles << std::endl;
  ...
  if(equal_mass) {
    if(gdimension == 1) {
      ...
      if(lattice_type == 0) {
        ...
        log_one(info) << std::endl
                      ...
                      << " - central box:      dx = " << sph_separation
                      << std::endl
                      << " - left/right boxes: dx = " << lr_sph_sep
                      << std::endl;
      }
    ...
    if(periodic_boundary_x or periodic_boundary_y or periodic_boundary_z) {
      ...
      if constexpr(gdimension >= 2)
        log_one(warn)
          << "Lattice mismatch, Y-direction:" << std::endl
          << " - central box: "
  ...
  if(equal_separation) {
    for(int64_t part = 0; part < tparticles; ++part) {
      ...
      if(particle_lattice::in_domain_1d(
           x[part], cbox_min[0], cbox_max[0], domain_type)) {
        P[part] = pressure_1;
        rho[part] = rho_1;
        vx[part] = vx_1;
