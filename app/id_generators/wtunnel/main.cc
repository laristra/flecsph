 ...
 * y and z directions. It is initialized in a section of the tunnel upstream
...
print_usage() {
  ...
  log_one(warn) << "Initial data generator for the wind tunnel test in"
                << gdimension << "D" << endl
                << "Usage: ./wtunnel_" << gdimension
                << "d_generator <parameter-file.par>" << endl;
}
...
main(int argc, char * argv[]) {
  ...
  // screen output
  log_one(info) << "Wind tunnel problem in " << gdimension << "D:" << endl
  ...
  tparticles = particle_lattice::count(
    lattice_type, 0, cbox_min, cbox_max, sph_separation, 0);
  ...
  tparticles = particle_lattice::generate(
    lattice_type, 0, cbox_min, cbox_max, sph_separation, 0, x, y, z);
