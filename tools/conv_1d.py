...
########################
my_description = """
Reads in a FleCSPH hdf5 file and produces its 1D ASCII reduction."""
my_usage="""
   %(prog)s [-h] [-x|-y|-z] ifile [-s|--step <val>] [-v|--var <val>]
...
parser.add_argument('-d', action='store', type=float, default=0.0, 
    help="""only output particles within stripe (in 2D) or cylinder (in 3D)
            of width D (default: output all particles)""")
...
if (args.x + args.y + args.z == 0):
  args.x = True

if (args.x + args.y + args.z != 1):
  sys.exit ("ERROR: only one of the options [-x|-y|-z] is allowed")
...
if args.d == 0.0 or ndim == 1:
  ...
  if args.y:
    ...
    if ndim == 2:
      ...
      for i in range(Npart):
        if (x[i]-x0)**2 + (z[i]-z0)**2 < d4:
          print ("%12.5e %12.5e %12.5e" % (y[i], var1[i], var2[i]))
  
  if args.z:
    print ("# 1:z 2:%s 3:%s" % (args.var1, args.var2))
    for i in range(Npart):
      if (x[i]-x0)**2 + (y[i]-y0)**2 < d4:
        print ("%12.5e %12.5e %12.5e" % (z[i], var1[i], var2[i]))
