...
protected:
  ...
  static constexpr size_t max_depth_ = (bits_-1)/dimension; //! Maximum
...
public:
  ...
  void pop() { assert(depth() > 0); value_ >>= dimension; }
...
template<size_t D, typename T, class DER> std::ostream& operator<<(
  std::ostream& ostr, const filling_curve<D,T,DER>& k)
{
  k.output_(ostr);
  return ostr;
}
...
public:
  ...
  : hilbert_curve(range,p,filling_curve<DIM,T,hilbert_curve>::max_depth_) {}
  ...
  void coordinates(const std::array<point_t, 2>& range, point_t& p)
  {
    ...
    for(size_t j = 0; j < dimension; ++j){
      ...
      p[j] = min + scale * static_cast<double>(coords[j])/(int_t(1)
        << max_depth_);
    } // for
  }
  ...
  { return std::array<point_t,2>{}; } // range
