...
//
  ...
  rank|| clog(trace)<<"MacAngle="<<macangle<<std::endl;
  ...
  { 
    ...
    rank|| clog(trace)<<"Total time="<<physics::totaltime<<"s / "<<maxtime<<"s"
      <<std::endl;
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    rank|| clog(trace)<<"Iteration time = "<<omp_get_wtime()-start_iteration<< "s" 
      <<std::endl;
