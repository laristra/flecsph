...
print_usage() {
  logm(warn)
    << "Change the velocity for relaxed simulation" << gdimension << "D"
    << std::endl
...
main(int argc, char * argv[]) {
  ...
  io::inputDataHDF5(
    bodies, argv[1], argv[2], totalnbodies, nbodies, atoi(argv[3]));
  ...
  for(auto & b : bodies) {
    ...
    vel[1] =
      0.01 * (1 + cos(4 * M_PI * pos[0])) * (1 + cos(3 * M_PI * pos[1])) / 4.;
