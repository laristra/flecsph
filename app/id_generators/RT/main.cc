...
set_derived_params() {
  ...
  bbox_min[1] += (box_width / 2.) - ((int)(box_width / 2. / dy)) * dy;
  ...
  if(periodic_boundary_x or periodic_boundary_y or periodic_boundary_z) {
    log_one(warn) << "Domain has been adjusted for periodic boundaries."
                  << std::endl;
    log_one(warn) << "For evolution, modify domain dimensions as follows:"
                  << std::endl
    ...
    log_one(warn) << "Lattice mismatch, X-direction:" << std::endl
                  << " -    top box: "
                  << (box_length -
                       floor((tbox_max[0] - tbox_min[0]) / dx_t) * dx_t)
                  << ", dx = " << dx_t << ", mismatch/dx = "
                  << (box_length / dx_t -
                       floor((tbox_max[0] - tbox_min[0]) / dx_t))
                  << std::endl
                  << " - bottom box: "
    ...
    if constexpr(gdimension >= 3)
      log_one(warn)
        << "Lattice mismatch, Z-direction:" << std::endl
        << " -    top box: "
...
main(int argc, char * argv[]) {
  ...
  // screen output
  log_one(info) << "Rayleigh-Taylor instability initial data "
                << "in " << gdimension << "D" << std::endl;
  ...
  double cs =
    sqrt(poly_gamma * std::max(pressure_1 / rho_1, pressure_2 / rho_2));
  ...
  for(int64_t part = 0; part < nparticles; ++part) {
    id[part] = posid++;
    if(particle_lattice::in_domain_1d(
         y[part], bbox_min[1], bbox_max[1], domain_type)) {
      rho[part] = rho_1;
      m[part] = pmass;
      P[part] =
        pressure_0 + gravity_acceleration_constant *
      ...
      m[part] = pmass;
      P[part] = pressure_0 +
                gravity_acceleration_constant * rho_2 * (tbox_max[1] - y[part]);
    }
