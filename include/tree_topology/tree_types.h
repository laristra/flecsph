...
public:
  ...
  tree_branch(
    const key_t& key,const point_t& coordinates,const element_t& mass,
    const point_t& bmin,const point_t& bmax,const b_locality& locality,
    const int& owner)
    : key_(key), coordinates_(coordinates), mass_(mass),
...
protected:
  ...
  friend std::ostream& operator<<(std::ostream& os, const tree_branch& b){
    ...
    os << " owner: "<<b.owner_ << " bit_child: "<<std::bitset<8>(b.bit_child_);
    return os;
  }
...
//----------------------------------------------------------------------------//
template<size_t D, typename E,class KEY, class ENT>
class tree_entity{
public:
...
public:
  ...
  tree_entity(
    const key_t& key,const point_t& coordinates,
