...
then
  echo "Error. Usage: "
  echo "./launch_weak_openMP.sh [sodtube,noh,sedov]"
  exit 1
fi
...
START_NPART=$(bc -l <<< "e(l($NPART_PER_CORE)/3)+1.5")
START_NPART=$(bc <<< "$START_NPART/1")
dir="./${INPUT}_weak_openMP_$START_NPART"
rm -rf $dir
mkdir $dir
cd $dir
...
for i in `seq 0 $max_ranks`; do
  for j in `seq 0 $max_omp`; do
    ...
    if [ $num_cores -le 36 ]; then
      NPARTS=$(bc -l <<< "e(l($NPART_PER_CORE*$num_cores)/3)+1.5")
      NPARTS=$(bc <<< "$NPARTS/1")
...
time mpirun -n 1 ${PATH_TO_FLECSPH_DIR}/build/app/id_generators/${INPUT}_3d_generator ./${INPUT}_n${NPARTS}.par
echo "Computation: RANKS ${num_ranks}, OMP_THREADS $num_omp"
time mpirun -n $num_ranks --bind-to socket --map-by socket ${PATH_TO_FLECSPH_DIR}/build/app/drivers/hydro_3d ./${INPUT}_n${NPARTS}.par
echo "Done: RANKS ${num_ranks}, OMP_THREADS $num_omp"
rm ev_${INPUT}_${NPARTS}.h5part
