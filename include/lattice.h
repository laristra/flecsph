 ...
 *
 * Module for generating lattices of different types, suitable for a wide
 * variety of problems, making it essential building block for initial data
 * construction.
 *
 * Having dimension set in the user.h file, this function will currently
 * populate either a rectangular lattice, a triangular lattice, an FCC lattice,
 * or a HCP lattice.
 *
 * Inputs:
 ...
 *  bbox_min     - bounding box: bottom-left corner (or a bounding cube if
 * domain) bbox_max     - bounding box: top-right corner   (is spherical )
 *  sph_sep      - particle separation within the lattice
 *  posid        - enter the particle ID number from which the function is
 * called it is important for when the function is assigning position
 *                 coordinates to the particles
 *  count_only   - boolean for determing particle number (returned for
 * allocating proper arrays) or writing positions to those arrays x,y,z[]      -
 * the arrays to be filled for the positions of each particle
 */
...
in_domain_3d(const double x,
  ...
  if(domain_type == 1) {
    const double r =
      0.5 * std::min(std::min(xmax - xmin, ymax - ymin), (zmax - zmin));
    const double x0 = 0.5 * (xmin + xmax);
    const double y0 = 0.5 * (ymin + ymax);
 ...
 *             depending on the count_only switch--count total number of
 * particles or assign the positions to the position arrays Returns int64_t:
 * total particle number
 *
 * @param      Refer to inputs section in introduction
 */
int64_t
generator_lattice_1d(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
...
int64_t
generator_lattice_2d(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
...
int64_t
generator_lattice_3d(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  ...
  if(lattice_type == 0) {
    ...
    for(double z_p = zmin, zo = 0; z_p < zmax; z_p += dz, zo = 1 - zo)
      for(double y_p = ymin - zo * dy / 3, yo = 0; y_p < ymax;
          y_p += dy, yo = 1 - yo)
    ...
    for(double z_p = zmin, zl = 0; z_p < zmax;
        z_p += dz, zl = (zl + 1) * (zl < 3))
      for(double y_p = ymin - zl * dy / 3, yo = 0; y_p < ymax;
          y_p += dy, yo = 1 - yo)
 ...
 */
int64_t
npart_icosahedral_shell(const int k) {
  int64_t N2 = 1;
  if(k > 0)
 ...
 */
int64_t
npart_icosahedral_sphere(const int k) {
  int64_t N3 = 1;
  if(k > 0)
 ...
 *             Depending on the count_only switch--count total number of
 * particles or assign the positions to the position arrays Uses current
 ...
 */
int64_t
generator_icosahedral_lattice(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  ...
  const double a2 = SQ(side_a), eta = sqrt(5 * sq3 / (4.0 * pi)) * side_a;

  int i, k, m, n, v1, v2, v3, NN, mm, m1, m2;
  double x1, x2, y1, y2, z1, z2, x_p, y_p, z_p, r, f, f1, f2, f3, ff[3], tan3;
  double xt, yt, x1t, y1t, Mk;
  double ico_vtx[12][3];
  const int ico_edg[30][2] = {{1, 2}, {1, 3}, {1, 4}, {1, 5}, {1, 6}, {2, 3},
    {3, 4}, {4, 5}, {5, 6}, {6, 2}, {2, 7}, {7, 3}, {3, 8}, {8, 4}, {4, 9},
    {9, 5}, {5, 10}, {10, 6}, {6, 11}, {11, 2}, {7, 8}, {8, 9}, {9, 10},
    {10, 11}, {11, 7}, {7, 12}, {8, 12}, {9, 12}, {10, 12}, {11, 12}};
  const int ico_fac[20][3] = {{1, 2, 3}, {1, 3, 4}, {1, 4, 5}, {1, 5, 6},
    {1, 6, 2}, {7, 2, 3}, {8, 3, 4}, {9, 4, 5}, {10, 5, 6}, {11, 6, 2},
      ...
      for(m = 1; m <= NN - 1; ++m) {
        for(n = 1; n <= NN - 1 - m; ++n) {
          ...
          if(y1t < 1e-8)
            ...
            x1t =
              xt * y1t *
              sqrt((1 + SQ(y1t)) / (SQ(yt) * (1 + 4 * SQ(y1t)) - SQ(xt * y1t)));

          ff[mm] = (1.0 - y1t * sqrt(12.0) / side_a) / 3.0;
          ff[m1] = (1.0 - ff[mm]) / 2.0 + x1t / side_a;
          ff[m2] = (1.0 - ff[mm]) / 2.0 - x1t / side_a;

          x_p = ff[1] * ico_vtx[v1][0] + ff[2] * ico_vtx[v2][0] +
                ff[0] * ico_vtx[v3][0];
          y_p = ff[1] * ico_vtx[v1][1] + ff[2] * ico_vtx[v2][1] +
                ff[0] * ico_vtx[v3][1];
          z_p = ff[1] * ico_vtx[v1][2] + ff[2] * ico_vtx[v2][2] +
                ff[0] * ico_vtx[v3][2];
 ...
 */
int64_t
generator_random_lattice(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  ...
  for(int i = 0; i < npart; i++) {
    ...
    while(rejected == true) {
      ...
      std::uniform_real_distribution<double> coord_gen(
      ...
      // Should we use a different generator here - density and coordinates
      // might not be truly uncorrelated.
      double random = density_gen(generator);
      double exact =
        density_profiles::spherical_density_profile(r / sphere_radius);

      if(random <= exact &&
         in_domain_3d(x_p, y_p, z_p, bbox_min, bbox_max, domain_type)) {
        if(!count_only) {
          x[posid] = x_p;
          y[posid] = y_p;
          z[posid] = z_p;
...
int64_t
generate_lattice_1d(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  ...
  double * y,
  double * z) {
  return generator_lattice_1d(lattice_type, domain_type, bbox_min, bbox_max,
    sph_sep, posid, false, x, y, z);
}
int64_t
count_lattice_1d(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  int64_t posid) {
  return generator_lattice_1d(
    lattice_type, domain_type, bbox_min, bbox_max, sph_sep, posid);
}

int64_t
generate_lattice_2d(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  ...
  double * y,
  double * z) {
  return generator_lattice_2d(lattice_type, domain_type, bbox_min, bbox_max,
    sph_sep, posid, false, x, y, z);
}
int64_t
count_lattice_2d(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  int64_t posid) {
  return generator_lattice_2d(
    lattice_type, domain_type, bbox_min, bbox_max, sph_sep, posid);
}

int64_t
generate_lattice_3d(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  ...
  double * y,
  double * z) {
  return generator_lattice_3d(lattice_type, domain_type, bbox_min, bbox_max,
    sph_sep, posid, false, x, y, z);
}
int64_t
count_lattice_3d(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  int64_t posid) {
  return generator_lattice_3d(
    lattice_type, domain_type, bbox_min, bbox_max, sph_sep, posid);
}

int64_t
generate_icosahedral_lattice(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  ...
  double * y,
  double * z) {
  return generator_icosahedral_lattice(lattice_type, domain_type, bbox_min,
    bbox_max, sph_sep, posid, false, x, y, z);
}
int64_t
count_icosahedral_lattice(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  int64_t posid) {
  return generator_icosahedral_lattice(
    lattice_type, domain_type, bbox_min, bbox_max, sph_sep, posid);
}

int64_t
generate_random_lattice(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  ...
  double * y,
  double * z) {
  return generator_random_lattice(lattice_type, domain_type, bbox_min, bbox_max,
    sph_sep, posid, false, x, y, z);
}
int64_t
count_random_lattice(const int lattice_type,
  const int domain_type,
  const point_t & bbox_min,
  const point_t & bbox_max,
  const double sph_sep,
  int64_t posid) {
  return generator_random_lattice(
    lattice_type, domain_type, bbox_min, bbox_max, sph_sep, posid);
}

// pointer types
typedef int64_t (*lattice_generate_function_t)(const int,
  const int,
  const point_t &,
  const point_t &,
  const double,
  ...
  double *,
  double *);
typedef int64_t (*particle_count_function_t)(const int,
  const int,
  const point_t &,
  const point_t &,
  const double,
